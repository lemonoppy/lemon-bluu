  ...
  for (let t = 0; t < trials; t++) {
    ...
    for (let round = 1; round <= roundCount; round++) {
      ...
      for (let i = 0; i < pairCount; i++) {
        ...
        if (shouldDraw) {
          draws[a]++; draws[b]++;
          points[a] += drawPoints; points[b] += drawPoints;
          intentionalDraws++;
        } else if (Math.random() < 0.5) {
          wins[a]++; points[a] += winPoints; losses[b]++;
        } else {
          wins[b]++; points[b] += winPoints; losses[a]++;
        }
      }
      if (n % 2 === 1) {
        const bye = order[n - 1];
    ...
    // New: update the worst (loosest) record ever seen making the cut
